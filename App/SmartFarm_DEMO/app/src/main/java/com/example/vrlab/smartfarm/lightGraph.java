...
public class lightGraph {
    ...
    public lightGraph(LineChart mLineChart, TextView TextView_empty)
    {
    ...
    public void Graph(String date)
    {
        ...
        for(i = 0; i<Menu.infoList.size(); i++)
        {
            if(DATE.equals(Menu.infoList.get(i).get(TAG_DATE_INFO)))
                valsComp1.add(new Entry(i, Float.parseFloat(Menu.infoList.get(i).get(TAG_ILLUMINATION))));
        }
        LineDataSet setComp1 = new LineDataSet(valsComp1, "조도");
        setComp1.setColor(Color.rgb(255, 187, 51));
        setComp1.setCircleColor(Color.rgb(255, 187, 51));
