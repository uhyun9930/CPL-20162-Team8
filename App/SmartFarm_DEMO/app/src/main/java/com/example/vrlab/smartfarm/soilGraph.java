...
public class soilGraph {
    ...
    public soilGraph(LineChart mLineChart, TextView TextView_empty)
    {
    ...
    public void Graph(String date)
    {
        ...
        for(i = 0; i<Menu.infoList.size(); i++)
        {
            if(DATE.equals(Menu.infoList.get(i).get(TAG_DATE_INFO)))
                valsComp1.add(new Entry(i, Float.parseFloat(Menu.infoList.get(i).get(TAG_SOIL_HUMID))));
        }
        LineDataSet setComp1 = new LineDataSet(valsComp1, "토양습도");
        setComp1.setColor(Color.rgb(153, 76, 0));
        setComp1.setCircleColor(Color.rgb(153, 76, 0));
